...
(async function () {
  ...
  try {
    ...
    if (storedLoadouts)
      loadouts = { ...DEFAULT_LOADOUTS, ...JSON.parse(storedLoadouts) };
  } catch (e) {
  ...
  function getPlayerHP() {
    ...
    if (parts.length === 2) {
      return {
        current: parseInt(parts[0].replace(/,/g, "")),
  ...
  function getUniquePowersCount() {
    const powerCards = document.querySelectorAll(
      ".side-column .power-list .power",
    );
  ...
  function initUI() {
    ...
    const createField = (labelHTML, inputHTML) => `
            <div style="display: flex; justify-content: space-between; align-items: center; gap: 6px;">
                <label style="font-size: 13px; color: #ffe3ad; font-weight: bold; white-space: nowrap;">${labelHTML}</label>
                <div style="flex: 1; max-width: 60%;">${inputHTML}</div>
            </div>
        `;

    const selectStyle = `width: 100%; border: 1px solid rgba(241, 201, 107, 0.34); border-radius: 6px; background: #120d16; color: #f8ead2; padding: 4px 6px; font: 700 12px Georgia, serif;`;

    body.innerHTML = `
            <div style="font-size: 11px; text-transform: uppercase; color: #7ea7ff; letter-spacing: 1px; border-bottom: 1px solid #333; padding-bottom: 4px;">Combat & Movement</div>
            ...
            ${createField(
              "Stops on",
              `<select id="vc-stops-at" style="${selectStyle}">
            ...
            ${createField("Pathing", `<select id="vc-event-choice" style="${selectStyle}"><option value="random">Random</option><option value="left">Left</option><option value="right">Right</option></select>`)}
            ...
            ${createField(
              ...
              `<select id="vc-door-choice" style="${selectStyle}">
                ...
                <option value="black">Black Door</option>
                <option value="silver">Silver Door</option>
                <option value="crimson">Crimson Door</option>
            </select>`,
            )}
            ${createField(
              "Shrine",
              `<select id="vc-shrine-choice" style="${selectStyle}">
                <option value="random">Random Blessing</option>
                <option value="heal">Drink From Chalice</option>
                <option value="cleanse">Cleanse Bad Status</option>
                <option value="bless">Bless Next Hit</option>
            </select>`,
            )}
            ${createField(
              "Mirror",
              `<select id="vc-mirror-choice" style="${selectStyle}">
            ...
            <div style="display: flex; flex-direction: column; gap: 8px; border: 1px solid rgba(241, 201, 107, 0.22); border-radius: 8px; padding: 10px; background: rgba(241, 201, 107, 0.07); margin-top: 8px;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: flex-start; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 8px; min-height: 20px;">
                <div id="vc-status-msg" style="color: #cdbfba; font-size: 12px; line-height: 1.4;">Ready.</div>
                <div id="vc-max-dmg" style="color: #ff9999; font-size: 12px; font-weight: bold; white-space: nowrap;">Max DMG: 0</div>
            </div>
        `;
    ui.appendChild(body);
  ...
  function bindUIEvents() {
    ...
    // Sync State
    getEl("vc-attack-type").onchange = (e) => {
      state.attackType = e.target.value;
      saveState();
    };
    getEl("vc-stops-at").onchange = (e) => {
      state.stopsAt = e.target.value;
      saveState();
    };
    getEl("vc-event-choice").onchange = (e) => {
      state.eventChoice = e.target.value;
      saveState();
    };

    // New Events
    getEl("vc-door-choice").onchange = (e) => {
      state.doorChoice = e.target.value;
      saveState();
    };
    getEl("vc-shrine-choice").onchange = (e) => {
      state.shrineChoice = e.target.value;
      saveState();
    };
    ...
    getEl("vc-potion-enabled").onchange = (e) => {
      state.potionEnabled = e.target.checked;
      saveState();
    };
    getEl("vc-continuous-runs").onchange = (e) => {
      state.continuousRuns = e.target.checked;
      saveState();
    };
    ...
    // Buttons
    getEl("vc-start-btn").onclick = () => {
      state.active = true;
      saveState();
      scheduleAutoStep();
    };
    getEl("vc-stop-btn").onclick = () => {
      stopAuto("User stopped.");
    };

    // Modal
    getEl("vc-settings-btn").onclick = () => {
      getEl("veyra-settings-modal").style.display = "block";
    };
    getEl("vc-close-modal").onclick = () => {
      getEl("veyra-settings-modal").style.display = "none";
    };
    ...
    getEl("vc-rename-loadout").onclick = async () => {
      const newName = prompt(
        "Enter new name for this loadout:",
        loadouts[state.currentLoadoutId].name,
      );
  ...
  function updateLoadoutDropdown() {
    ...
    select.innerHTML = Object.entries(loadouts)
      .map(
        ([id, data]) =>
  ...
  function populateModal() {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      num.innerText = `${i + 1}.`;
      num.style.cssText =
        "width: 20px; color: #d9b66f; font-weight: bold; text-align: right;";
  ...
  // Form Submitter wrapper. If the target is just a button and not a form, it clicks it.
  async function submitFormThenReload(target) {
    if (!target || isActionInFlight) return;
      ...
      if (data.message) {
        ...
        if (match) {
          ...
          if (dmg > maxDmgDealt) {
            ...
            if (dmgEl)
              dmgEl.innerText = `Max DMG: ${maxDmgDealt.toLocaleString()}`;
          }
        }
      }
  ...
  function scheduleAutoStep() {
    ...
    // 1. Check End of Run
    const startRunForm = document.querySelector(
      'form input[value="start_run"]',
    );
    if (startRunForm) {
    ...
    if (leftForm && rightForm) {
      ...
      if (state.eventChoice === "random")
        target = Math.random() < 0.5 ? leftForm : rightForm;
    ...
    if (isEventActive) {
      ...
      const silverDoor = getEventFormByChoice("silver");
      const crimsonDoor = getEventFormByChoice("crimson");
      if (blackDoor && silverDoor && crimsonDoor) {
        setStatus("Whispering Door Event...");
        let target;
        if (state.doorChoice === "black") target = blackDoor;
        else if (state.doorChoice === "silver") target = silverDoor;
        else if (state.doorChoice === "crimson") target = crimsonDoor;
        else {
          const arr = [blackDoor, silverDoor, crimsonDoor];
      ...
      // Shrine of the Last Guest
      const drinkForm = getEventFormByChoice("heal");
      const cleanseForm = getEventFormByChoice("cleanse");
      const blessForm = getEventFormByChoice("bless");
      if (drinkForm && cleanseForm && blessForm) {
        setStatus("Shrine Event...");
        let target;
        if (state.shrineChoice === "heal") target = drinkForm;
        else if (state.shrineChoice === "cleanse") target = cleanseForm;
        else if (state.shrineChoice === "bless") target = blessForm;
        else {
          const arr = [drinkForm, cleanseForm, blessForm];
          target = arr[Math.floor(Math.random() * arr.length)];
        }
        submitFormThenReload(target);
        return;
      ...
      // Cursed Mirror
      const fightForm = getEventFormByChoice("fight");
      const walkForm =
        getEventFormByChoice("walk") || getFormByButtonText("Walk Away");
      if (fightForm && walkForm) {
        setStatus("Cursed Mirror Event...");
        let target;
        if (state.mirrorChoice === "fight") target = fightForm;
        else if (state.mirrorChoice === "walk") target = walkForm;
        else target = Math.random() < 0.5 ? fightForm : walkForm;
        submitFormThenReload(target);
        return;
      }
    }
    ...
    // 6. Attack
    const attackForms = document.querySelectorAll(
      'form input[name="action"][value="attack"]',
    );
            ...
            if (data.monster.hp) {
              ...
              if (hpText)
                hpText.innerText =
                  data.monster.hp +
                  " / " +
                  (getPlayerHP()?.max || data.monster.hp);
            }
            ...
            if (data.player.hp) {
              ...
              if (hpText)
                hpText.innerText =
                  data.player.hp +
                  " / " +
                  (getPlayerHP()?.max || data.player.hp);
            }
  ...
  function setReplaceSelect(form, discardName, discardRarity) {
    ...
    const targetRk =
      keywords.find((k) => discardRarity.toLowerCase().includes(k)) || "";
    ...
    if (!bestOpt) {
      for (let opt of select.options) {
        if (opt.text.toLowerCase().trim().includes(dName)) {
          bestOpt = opt;
          break;
        }
      }
  ...
  function handlePowerSelection(forms) {
    ...
    const hasLegendary = forms.some((f) => {
      const h3 = f.querySelector("h3");
      return h3 && getRarityScore(h3.className) >= 5;
    });
    const hasEpic = forms.some((f) => {
      const h3 = f.querySelector("h3");
      return h3 && getRarityScore(h3.className) >= 4;
    });
    ...
    document
      .querySelectorAll(".side-column .power-list .power")
      .forEach((card) => {
    ...
    const loadout = loadouts[state.currentLoadoutId].priorities.filter(
      (p) => p !== "",
    );
    ...
    if (bestForm) {
      if (bestReplace) {
        const success = setReplaceSelect(
          bestForm,
          bestReplace.name,
          bestReplace.rarity,
        );
        if (!success) {
          stopAuto(
            "Failed to select replacement target for " + bestReplace.name,
          );
          return;
        }
      }
