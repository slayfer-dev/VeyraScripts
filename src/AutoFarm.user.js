  ...
  try {
    ...
    const MONSTERS = {
      ...
      "goblin skirmisher": {
        ...
        wave: 1,
        img: "images/monsters/monster_689bea482aecd5.59004851.webp",
      },
      "goblin slinger": {
        displayName: "Goblin Slinger",
        wave: 1,
        img: "images/monsters/monster_689e900b122598.86870107.webp",
      },
      "orc grunt": {
        displayName: "Orc Grunt",
        wave: 1,
        img: "images/monsters/monster_689e9049f06a39.75621837.webp",
      },
      "orc bonecrusher": {
        displayName: "Orc Bonecrusher",
        wave: 1,
        img: "images/monsters/monster_689e9075bed4e6.67265953.webp",
      },
      "hobgoblin spearman": {
        displayName: "Hobgoblin Spearman",
        wave: 1,
        img: "images/monsters/monster_689e90c0d497d8.56568541.webp",
      },
      // Wave 2
      "troll ravager": {
        displayName: "Troll Ravager",
        wave: 2,
        img: "images/monsters/monster_689e90e9ae1b88.40735056.webp",
      },
      "lizardman flamecaster": {
        displayName: "Lizardman Flamecaster",
        wave: 2,
        img: "images/monsters/monster_689e913ecffac3.17663892.webp",
      },
      "troll brawler": {
        displayName: "Troll Brawler",
        wave: 2,
        img: "images/monsters/monster_689e9180028033.05347656.webp",
      },
      "lizardman shadowclaw": {
        displayName: "Lizardman Shadowclaw",
        wave: 2,
        img: "images/monsters/monster_689e91b2965a08.22765337.webp",
      },
      // Wave 3 pending...
    };
    ...
    const THUNDER_SOUND_B64 = (() => {
      ...
      for (let i = 0; i < samples; i++) {
        ...
        const rumble =
          Math.sin(t * 80 * Math.PI) * 0.3 + Math.sin(t * 40 * Math.PI) * 0.2;
    ...
    function playThunder() {
      try {
        new Audio(THUNDER_SOUND_B64).play();
      } catch (e) {}
    }
    ...
    globalConfig.potionConfig = {
      ...DEFAULT_CONFIG.potionConfig,
      ...(globalConfig.potionConfig || {}),
    };
    if (!globalConfig.activeWaves)
      globalConfig.activeWaves = Object.keys(WAVES);
    ...
    async function saveGlobalConfig() {
      await GM.setValue("veyra_af2_config", globalConfig);
    }
    async function saveMonsterCfg() {
      await GM.setValue("veyra_af2_monster_config", monsterConfig);
    }
    async function saveHistory() {
      await GM.setValue("veyra_af2_history", history);
    }

    function saveMonsterConfigField(monsterName, key, value) {
      if (!monsterConfig[monsterName])
        monsterConfig[monsterName] = { ...DEFAULT_MONSTER_CONFIG };
    ...
    function appendStatus(text, cssClass, targetBox = "af2-farm-status") {
      ...
      p.className =
        "af2-status-line" + (cssClass ? " af2-status-" + cssClass : "");
    ...
    function recordHit(attackName, skillName, dmg, staminaUsed, mpUsed) {
      ...
      if (attackName) {
        activeEncounter.attacksUsed[attackName] =
          (activeEncounter.attacksUsed[attackName] || 0) + 1;
      }
      if (skillName) {
        activeEncounter.skillsUsed[skillName] =
          (activeEncounter.skillsUsed[skillName] || 0) + 1;
      }
    ...
    function trackLoot(exp, gold, items) {
      ...
      if (items && items.length > 0) {
        for (const item of items) {
          ...
          if (!history.loot.items[key])
            history.loot.items[key] = {
              qty: 0,
    ...
    async function setupUI() {
      ...
      container.querySelectorAll(".af2-tab").forEach((tab) => {
        tab.addEventListener("click", async () => {
          container
            .querySelectorAll(".af2-tab")
            .forEach((t) => t.classList.remove("active"));
          container
            .querySelectorAll(".af2-tab-content")
      ...
      container.querySelectorAll(".af2-wave-cb").forEach((cb) => {
        cb.addEventListener("change", async () => {
          globalConfig.activeWaves = Array.from(
            container.querySelectorAll(".af2-wave-cb:checked"),
          ).map((el) => el.value);
      ...
      document
        ...
        .addEventListener("click", async () => {
          ...
          btn.className =
            "af2-btn " + (isRunning ? "af2-btn-stop" : "af2-btn-start");
          if (isRunning) {
            loop();
          } else {
      ...
      document
        .getElementById("af2-scan-btn")
        .addEventListener("click", () => prefetchWaves("alive"));
      ...
      container.querySelectorAll(".af2-pill").forEach((pill) => {
        pill.addEventListener("click", async () => {
          ...
          globalConfig.lootSelectedMonsters = Array.from(
            container.querySelectorAll(".af2-pill.selected"),
          ).map((p) => p.dataset.monster);
          await saveGlobalConfig();
        });
      });

      document
        .getElementById("af2-loot-all")
        .addEventListener("click", () => doBatchLoot("all"));
      document
        .getElementById("af2-loot-selected")
        .addEventListener("click", () => doBatchLoot("selected"));
      document.getElementById("af2-loot-x").addEventListener("click", () => {
        const amount =
          parseInt(document.getElementById("af2-loot-x-amount").value) || 1;
        doBatchLoot("selected", amount);
      });
      document
        .getElementById("af2-scan-dead-btn")
        .addEventListener("click", async () => {
          appendStatus(
            "Force refreshing dead monsters...",
            "info",
            "af2-loot-status",
          );
          await prefetchWaves("dead");
      ...
      // ---- Config Controls ----
      document
        .getElementById("af2-cfg-sound")
        .addEventListener("change", async (e) => {
          globalConfig.soundStaminaOut = e.target.checked;
          await saveGlobalConfig();
        });
      document
        .getElementById("af2-cfg-keep-checking")
        .addEventListener("change", async (e) => {
          globalConfig.keepCheckingStamina = e.target.checked;
          document.getElementById("af2-cfg-check-interval-row").style.display =
            e.target.checked ? "flex" : "none";
          await saveGlobalConfig();
        });
      document
        .getElementById("af2-cfg-check-interval")
        .addEventListener("change", async (e) => {
          globalConfig.staminaCheckInterval = Math.max(
            1,
      ...
      document
        .getElementById("af2-cfg-standby-warn")
        .addEventListener("change", async (e) => {
      ...
      document
        .getElementById("af2-cfg-potions-global")
        .addEventListener("change", async (e) => {
          globalConfig.potionsEnabled = e.target.checked;
          document.getElementById("af2-potions-container").style.display = e
            .target.checked
            ? "block"
            : "none";
          await saveGlobalConfig();
      ...
      container.querySelectorAll(".af2-potion-enable").forEach((cb) => {
        cb.addEventListener("change", async (e) => {
          ...
          if (!globalConfig.potionConfig[key])
            globalConfig.potionConfig[key] = {};
          globalConfig.potionConfig[key].enabled = e.target.checked;
          container.querySelector(
            `.af2-potion-details[data-key="${key}"]`,
      ...
      container.querySelectorAll(".af2-potion-threshold").forEach((slider) => {
        slider.addEventListener("input", (e) => {
          e.target
            .closest(".af2-potion-details")
            .querySelector(".af2-slider-val").textContent =
            e.target.value + "%";
        });
      ...
      container.querySelectorAll(".af2-potion-reserve").forEach((input) => {
        input.addEventListener("change", async (e) => {
          const key = e.target.dataset.key;
          globalConfig.potionConfig[key].minReserve = Math.max(
            0,
      ...
      // ---- History ----
      document
        .getElementById("af2-clear-history")
        .addEventListener("click", async () => {
    ...
    function renderFarmMonsters() {
      ...
      const attackOpts = Object.entries(ATTACKS)
        .map(
          ([id, a]) =>
      ...
      for (const [waveQuery, waveInfo] of Object.entries(WAVES)) {
        ...
        const count = Object.values(waveGroup).reduce(
          (sum, m) => sum + m.ids.length,
          0,
        );
        ...
        for (const [dataName, m] of Object.entries(waveGroup)) {
          ...
          const imgSrc =
            m.img ||
            (MONSTERS[dataName]?.img
              ? `https://demonicscans.org/${MONSTERS[dataName].img}`
              : "");

          const selAttack = attackOpts.replace(
            `value="${conf.attackId}"`,
            `value="${conf.attackId}" selected`,
          );
          const selSkill = skillOpts.replace(
            `value="${conf.skillId}"`,
            `value="${conf.skillId}" selected`,
          );
      ...
      if (!foundAny) {
        container.innerHTML =
          '<div style="font-size:12px; color:#8a80a0; padding-top:10px;">No alive monsters found. Click Scan to refresh.</div>';
      }

      // Attach events
      container
        .querySelectorAll(".af2-m-enable")
        .forEach((el) =>
          el.addEventListener("change", (e) =>
            saveMonsterConfigField(
              e.target.dataset.id,
              "enabled",
              e.target.checked,
            ),
    ...
    function renderLootMonsters() {
      ...
      if (!foundAny) {
        container.innerHTML =
          '<div style="font-size:12px; color:#8a80a0; padding-top:10px;">No dead monsters found to loot.</div>';
      }
    }
    ...
    function renderHistory() {
      ...
      if (history.encounters.length === 0) {
        encHtml =
          '<div style="font-size:12px; color:#8a80a0; text-align:center; padding:10px;">No encounters recorded yet.</div>';
      } else {
        for (const enc of history.encounters) {
          ...
          const imgSrc = monsterInfo?.img
            ? `https://demonicscans.org/${monsterInfo.img}`
            : "";
          const statusClass =
            enc.status === "dead"
              ? "af2-enc-status-dead"
              : "af2-enc-status-alive";
          const statusLabel = enc.status === "dead" ? "Dead" : "Alive";

          // Format attacks used
          const attacksList =
            Object.entries(enc.attacksUsed || {})
              .map(([name, count]) => `${name} x${count}`)
              .join(", ") || "None";
          const skillsList =
            Object.entries(enc.skillsUsed || {})
              .map(([name, count]) => `${name} x${count}`)
              .join(", ") || "None";
      ...
      encEl.querySelectorAll(".af2-enc-view-btn").forEach((btn) => {
        btn.addEventListener("click", () => {
          window.open(
            `https://demonicscans.org/battle.php?id=${btn.dataset.id}`,
            "_blank",
          );
        });
      });
    ...
    function showLootResultsModal(batchResults) {
      ...
      // Close
      modal
        .querySelector(".af2-loot-modal-close")
    ...
    async function prefetchWaves(mode = "alive") {
      ...
      for (const waveQuery of wavesToScan) {
        ...
        try {
          const html = await fetch(
            `https://demonicscans.org/active_wave.php?${waveQuery}`,
          ).then((r) => r.text());
          ...
          cards.forEach((card) => {
            ...
            if (!tempMonsters[waveQuery][dataName]) {
              tempMonsters[waveQuery][dataName] = {
                dataName,
                name: displayName,
                img,
                ids: [],
      ...
      if (isFarm) {
        ...
        renderLootMonsters();
        appendStatus(
          `Found ${Object.values(tempMonsters).reduce((s, wg) => s + Object.values(wg).reduce((s2, m) => s2 + m.ids.length, 0), 0)} dead monster(s).`,
          "good",
          "af2-loot-status",
        );
      }
    ...
    async function prefetchTargetStats(enemyId) {
      try {
        const html = await fetch(
          `https://demonicscans.org/battle.php?id=${enemyId}`,
        ).then((r) => r.text());
        const parser = new DOMParser();
        const doc = parser.parseFromString(html, "text/html");
        ...
        return {
          ...
          currentStamina: staminaEl
            ? Number(staminaEl.textContent.replace(/[^0-9]/g, ""))
            : 999,
        };
      } catch (e) {
    ...
    async function joinBattle(enemyId) {
      for (let j = 0; j < 3; j++) {
        try {
          ...
          if (
            result.includes("You are already part of this battle.") ||
            result.includes("successfully joined")
          )
            return true;
    ...
    async function checkAndUsePotion(
      resource,
      currentValue,
      maxValue,
      actionCost = 0,
      ...
      const relevantPotions = Object.entries(POTIONS).filter(
    ...
    async function attackEnemy(enemyId, dataName) {
      ...
      if (stats.currentStamina < attack.staminaCost) {
        const potionUsed = await checkAndUsePotion(
          "stamina",
        ...
        if (!potionUsed) {
          appendStatus(
            `Skipping ${MONSTERS[dataName]?.displayName || dataName}: Not enough stamina (${stats.currentStamina}/${attack.staminaCost})`,
            "bad",
          );
      ...
      while (isRunning) {
        ...
        if (stats.currentStamina < attack.staminaCost) {
          ...
          if (!potionUsed) {
            appendStatus(
              `Out of stamina for ${MONSTERS[dataName]?.displayName || dataName}`,
              "bad",
            );
        ...
        try {
          ...
          if (skill && skill.type === "attack" && conf.skillId !== "") {
            const mpUsed = skill.mpCost;
            const skillResult = await fetch(
              "https://demonicscans.org/damage.php",
              {
                headers: {
            ...
            if (skillResult.status === "error") {
              ...
              recordHit(
                null,
                skill.name,
                skillResult.totaldmgdealt || 0,
                0,
                mpUsed,
              );
              if (skillResult.hp?.value <= 0) {
                finalizeEncounter("dead");
                return { action: "dead" };
              }
              if (skillResult.totaldmgdealt >= conf.dmgThreshold) {
                finalizeEncounter("alive");
                return { action: "threshold" };
              }
            }
          ...
          if (result.status === "error") {
            ...
            if (result.message?.includes("inactivity")) {
              await joinBattle(enemyId);
              continue;
            }
          ...
          if (
            result.retaliation?.damage > 0 &&
            result.retaliation?.user_hp_after > 0
          ) {
            await checkAndUsePotion(
              "hp",
              result.retaliation.user_hp_after,
              21150,
            );
          }
          ...
          if (result.totaldmgdealt >= conf.dmgThreshold) {
            finalizeEncounter("alive");
            return { action: "threshold" };
          }
          if (result.hp?.value <= conf.dmgThreshold * 2) {
            finalizeEncounter("alive");
            return { action: "threshold" };
          }
    ...
    async function loop() {
      ...
      for (const target of targetInstances) {
        if (!isRunning) break;
        appendStatus(
          `Attacking ${MONSTERS[target.dataName]?.displayName || target.dataName}`,
          "action",
        );
        ...
        if (result && result.action === "dead") {
          appendStatus(
            `${MONSTERS[target.dataName]?.displayName || target.dataName} killed!`,
            "good",
          );
        } else if (result && result.action === "threshold") {
          appendStatus(
            `Threshold reached for ${MONSTERS[target.dataName]?.displayName || target.dataName}`,
            "info",
          );
        } else if (result === null) {
          if (!isRunning) break;
          if (globalConfig.keepCheckingStamina) {
            appendStatus(
              `Stamina exhausted. Will retry in ${globalConfig.staminaCheckInterval} min...`,
              "info",
            );
            if (globalConfig.soundStaminaOut) playThunder();
    ...
    async function doBatchLoot(mode, limit = Infinity) {
      ...
      for (const waveGroup of Object.values(deadMonsters)) {
        for (const [dataName, m] of Object.entries(waveGroup)) {
          if (
            mode === "selected" &&
            !globalConfig.lootSelectedMonsters.includes(dataName)
          )
            continue;
          for (const inst of m.ids) {
      ...
      if (toLoot.length === 0) {
        appendStatus(
          "No dead monsters match your selection.",
          "info",
          "af2-loot-status",
        );
        isLooting = false;
      ...
      appendStatus(
        `Looting ${toLoot.length} monster(s)...`,
        "action",
        "af2-loot-status",
      );
      ...
      for (let i = 0; i < toLoot.length; i++) {
        ...
        appendStatus(
          `[${i + 1}/${toLoot.length}] Looting ${MONSTERS[target.dataName]?.displayName || target.dataName}...`,
          "action",
          "af2-loot-status",
        );
      ...
      appendStatus(
        `Done! Looted: ${successCount}/${toLoot.length}`,
        "good",
        "af2-loot-status",
      );
      isLooting = false;
