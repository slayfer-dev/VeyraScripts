...
(function() {
    ...
    function saveUIState() {
        ...
        localStorage.setItem(DB_KEY + '_ui', JSON.stringify({ pos: uiPos, minimized: isMinimized }));
    }
    ...
    function scrapeSkills() {
        ...
        elements.forEach(el => {
            ...
            const name = el.getAttribute('data-skill-name') || el.querySelector('.skillName')?.innerText || "Unknown";
    ...
    function initUI() {
        ...
        const checkBounds = () => {
            ...
            if (newTop < 0) { newTop = 20; changed = true; }
            if (newTop > window.innerHeight - 50) { newTop = Math.max(20, window.innerHeight - 100); changed = true; }
            if (newLeft > window.innerWidth - 50) { newLeft = Math.max(20, window.innerWidth - 150); changed = true; }
            if (newLeft < -rect.width + 50) { newLeft = 20; changed = true; }
        ...
        header.addEventListener("pointerdown", (e) => {
            if (e.target.closest('#gsd-minimize') || e.target.closest('#gsd-refresh')) return; // Ignore drag on buttons
            isDragging = true;
            const rect = ui.getBoundingClientRect();
    ...
    function renderSkills() {
        ...
        skillKeys.forEach(id => {
            ...
            if (!tagsHtml) tagsHtml = `<div class="gsd-tag" style="color: #aaa;">No attributes found</div>`;

            // Fallback image if missing: transparent 1x1 GIF
            const imgSrc = s.imgUrl || 'data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=';
